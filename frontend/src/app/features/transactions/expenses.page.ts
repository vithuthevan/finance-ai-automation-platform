    ...
    <div class="page">
      ...
      @if (auth.hasRole('ADMIN', 'ACCOUNTANT', 'BUSINESS_OWNER')) {
        <form class="toolbar-row" [formGroup]="form" (ngSubmit)="create()">
          ...
          <mat-form-field><mat-label>Vendor</mat-label><input matInput formControlName="vendorName"></mat-form-field>
      ...
      <table mat-table [dataSource]="rows" class="full-width">
        ...
        <ng-container matColumnDef="vendor"><th mat-header-cell *matHeaderCellDef>Vendor</th><td mat-cell *matCellDef="let row">{{ row.vendorName }}</td></ng-container>
...
export class ExpensesPage implements OnInit {
  ...
  create(): void {
    this.api.post(`/api/v1/clients/${this.clientId}/expenses`, this.form.getRawValue()).subscribe(() => this.reload());
  }

  approve(row: any): void {
    this.api.post(`/api/v1/clients/${this.clientId}/expenses/${row.id}/approve`).subscribe(() => this.reload());
  }

  voidRow(row: any): void {
    this.api.post(`/api/v1/clients/${this.clientId}/expenses/${row.id}/void`, { reason: 'Voided from workspace' }).subscribe(() => this.reload());
  }
