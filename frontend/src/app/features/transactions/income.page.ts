...
@Component({
  ...
  template: `
    ...
    <div class="page">
      ...
      <app-page-header
        title="Income"
        subtitle="Create and approve income drafts for the active client. Use the header to switch clients." />
          ...
          <form [formGroup]="form" (ngSubmit)="create()" novalidate>
            ...
            <div class="form-grid">
              ...
              <mat-form-field appearance="outline">
                ...
                @if (form.controls.customerName.touched && form.controls.customerName.hasError('required')) {

                  <mat-error>Customer is required</mat-error>

                } @else if (form.controls.customerName.touched && form.controls.customerName.hasError('maxlength')) {

                  <mat-error>Customer must not exceed 200 characters</mat-error>
              ...
              <mat-form-field appearance="outline">
                ...
                @if (form.controls.paymentMethod.touched && form.controls.paymentMethod.invalid) {
      ...
      @if (loadError) {
        <app-error-state title="Could not load income" [message]="loadError" (retry)="reload()" />
      } @else if (loading) {
        <app-loading-state message="Loading income…" />
      } @else if (!rows.length) {
        <app-empty-state
...
export class IncomePage implements OnInit {
  ...
  columnDefs: ColDef[] = [
    ...
    { headerName: 'Customer', field: 'customerName', minWidth: 140 },
...

    {
      ...
      cellRenderer: (p: ICellRendererParams) => {
        ...
        const parent = p.context.parent as IncomePage;
  ...
  reload(): void {
    ...
    this.api.get<any>(`/api/v1/clients/${this.clientId}/income`, {
      ...
      error: () => {
        this.rows = [];
        this.loadError = 'Unable to load income.';
      }
    });
  ...
  create(): void {
    ...
    this.api.post(`/api/v1/clients/${this.clientId}/income`, {
      ...
      next: () => {
        ...
        this.toast.success('Income draft created');
      ...
      error: (err) => {
        ...
        this.toast.error(err.error?.detail || 'Unable to create income');
  ...
  approve(row: any): void {
    ...
    this.api.post(`/api/v1/clients/${this.clientId}/income/${row.id}/approve`).subscribe({

      next: () => {

        this.approvingId = null;

        this.toast.success('Income approved');
  ...
  voidRow(row: any): void {
    ...
    this.api.post(`/api/v1/clients/${this.clientId}/income/${row.id}/void`, { reason: 'Voided from workspace' }).subscribe({

      next: () => {

        this.voidingId = null;

        this.toast.success('Income voided');
